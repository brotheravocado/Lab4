...
int main(int argc, char *argv[])
{
    ...
    g_signal_connect(plusbutton, "clicked", G_CALLBACK(symbol_check), (gpointer)'+');
    g_signal_connect(minusbutton, "clicked", G_CALLBACK(symbol_check), (gpointer)'-');
    g_signal_connect(dividebutton, "clicked", G_CALLBACK(symbol_check), (gpointer)'*');
    g_signal_connect(multiplybutton, "clicked", G_CALLBACK(symbol_check), (gpointer)'*');
    g_signal_connect(total, "clicked", G_CALLBACK(submit), (gpointer)'=');
    g_signal_connect(clear, "clicked", G_CALLBACK(clear), (gpointer)'d');
    g_signal_connect(press_one_del, "clicked", G_CALLBACK(delete_event), (gpointer)'o');
...
void symbol_check(GtkWidget *widget, gpointer data)
{
    if (has_press_symbol == 0)
    {
        if (second_num == -1 && first_num == -1 && press_number == 0)
        {
        }
        else if (second_num == -1 && first_num == -1 && press_number == 1)
        {
            first_num = get_entry_value();
            press_number = 0;
        }
        else if (second_num == -1 && first_num != -1 && press_number == 1)
        {
            second_num = get_entry_value();
            first_num = calculator(symbol);
            second_num = -1;
            char *result_string = malloc(sizeof(char) * 10);
            memset(result_string, 0, 10);
            sprintf(result_string, "%d", first_num);
            gtk_entry_set_text(GTK_ENTRY(user_entry), result_string);
        }
        symbol = (char)data;
        need_clear = 1;
        has_press_symbol = 1;
